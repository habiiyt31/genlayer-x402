...
Load `contracts/x402_subscription.py`, deploy with:
...
> **Key insight:** Subscriber count stays at 1 because same user top-up. Total periods sold increases.
...
- **Method:** `grant_access`
- **Input:** `user_address`: Subscriber's address, `periods`: `1`
- **Expected:** ✅ Success

Verify: `get_remaining_calls(user_address)` → `"17"` (12 + 5 granted)

#### Step 23: `update_price(new_price)`
...
#### Step 24: `update_calls_per_period(new_count)`
...
#### Step 25: Grant to a fresh account

- **Method:** `grant_access`
- **Input:** `user_address`: new account address, `periods`: `1`
- After `update_calls_per_period(10)` → 1 period × 10 = 10 free calls
- **Expected:** ✅ Success

#### Step 26: Verify Subscriber Count Increments
...
#### Step 27: Subscribe with 0 periods

**[Subscriber account]**

- **Method:** `subscribe`, **Input:** `periods`: `0`, **Value:** `1`
- **Expected:** ❌ ERROR: `"Must buy at least 1 period"`

#### Step 28: Subscribe with Insufficient Value

After price update to 2 GEN, try paying only 1 GEN:

- **Method:** `subscribe`, **Input:** `periods`: `1`, **Value (GEN):** `1`
- **Expected:** ❌ ERROR: `"x402: Insufficient. Required ... wei"`

#### Step 29: get_data without subscription
...
#### Step 30: Non-owner grant_access
...
#### Step 31: Non-owner update_price
...
#### Step 32: Use all remaining calls

Call `get_data` repeatedly until `remaining_calls` = 0.

#### Step 33: Verify no access after exhaustion

- `is_active(user_address)` → `false` ✅

#### Step 34: Try get_data when quota = 0
...
#### Step 35: Partial withdraw

- **Method:** `withdraw`, **Input:** `amount`: `1000000000000000000`
- **Expected:** ✅ Success

#### Step 36: Full withdraw

- **Method:** `withdraw_all`
- **Expected:** ✅ Success

#### Step 37: Verify empty
...
- Step 9: "Subscriptions STACK — users can top up anytime without losing existing quota"
...
```
new_remaining = old_remaining + (periods_bought × calls_per_period)
```
